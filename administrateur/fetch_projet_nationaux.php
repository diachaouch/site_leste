...
?>
	<div class="table table-responsive ">
		<button type="button" name="add" id="add" class="btn btn-info" class="btn btn-info" data-toggle="collapse" data-target="#user_dialog">Ajouter un projet national </button>
<br/>
						<div class="container">
						<div id="display"> </div>
			...
			<div class="form-group ">
					...
					<input type="text"  id="data4" class="form-control" placeholder="Responsable *" />
					<span id="error_first_name" class="text-danger"></span>
				
			</div>
			<div class="form-group ">
					
					<input type="text"  id="data5" class="form-control" placeholder="Structure porteuse *" />
					<span id="error_first_name" class="text-danger"></span>
				</div>
...
$query = "SELECT *  FROM projets_nationaux  ";
 $result = mysqli_query($connect, $query);
		  ...
		  echo '
              <tr id= '.$row["id"].'>
                ...
                <td><button class="btn btn-success btn-sm"><a class="text-light" href="update_projet_national.php?deleteid='.$row['id'].' ">Modifier</a></button> </td>
...
<script src="ckeditor/ckeditor.js"></script>
 ...
 {
  $.ajax({
   url:"fetch_projet_nationaux.php",
   method:"POST",
   ...
   function deleteAjax(id){
       ...
       if(confirm('Vous étes sure?')){
         ...
         $.ajax({
              ...
              type:'post',
              url:'delete_projet_nationaux.php',
			  
              data:{delete_id:id},
              success:function(data){
              
                   $('#delete'+data.id).hide('slow');
				   $('#inserted_item_data').fadeIn('slow').load("fetch_projet_nationaux.php");
 ...
 $("#submit").click(function(){
	...
	if( type=='' ){
	  ...
	  $.ajax({
     url:"insert_projet_national.php",
     method:"POST",
     data: dataString,
	 cache:false,
