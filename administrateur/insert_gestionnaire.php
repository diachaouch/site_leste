...
$q1="select * from users WHERE (login='$login' AND mot_de_passe='$mot_de_passe') AND (email='$email') ";
 ...
 $q="select * from gestionnaire WHERE (login='$login' AND mot_de_passe='$mot_de_passe')
 OR (email='$email') ";
  ...
  $rows=mysqli_num_rows($log);
	...
	echo 'Gestionnaire déja inscrit ';	
		...
		$query = "INSERT INTO gestionnaire (name,  email , telephone, login, mot_de_passe, user_id) VALUES(
'$name'  , '$email' , '$telephone'  , '$login', '$mot_de_passe' , '$user_id')";
          if(mysqli_query($connect, $query))
          {
          echo "Gestionnaire ajoutè  avec succès";
